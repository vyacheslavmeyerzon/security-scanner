...
from typing import Dict, List, NamedTuple, Pattern, Optional
...
class PatternMatcher:
    ...
    def _load_default_patterns(self) -> List[PatternDefinition]:
        ...
        patterns = [
            ...
            PatternDefinition(
                name="PayPal Token",
                pattern=re.compile(r'access_token\$production\$[a-z0-9]{16}\$[a-z0-9]{32}'),
                severity=Severity.CRITICAL,
            ...
            PatternDefinition(
                ...
                pattern=re.compile(r'(?:export\s+)?(?P<key>[A-Z_]{2,}[A-Z0-9_]*)=(?P<value>[^\s]+)'),
                severity=Severity.LOW,
                description="Environment variable assignment",
                false_positive_check=re.compile(r'^(PATH|HOME|USER|SHELL|TERM|LANG|LC_|PWD|OLDPWD)$')
            ),
        ]
        return patterns
    ...
    def _load_exclusion_patterns(self) -> List[Pattern[str]]:
        ...
        return [
            ...
            re.compile(r'\.(jpg|jpeg|png|gif|ico|svg|pdf|zip|tar|gz|rar)$', re.IGNORECASE),
                ...
                for match in matches:
                    ...
                    if pattern_def.false_positive_check:
                        if pattern_def.name == "Environment Variable":
                            key_match = match.group('key') if 'key' in match.groupdict() else match.group(0)
    ...
    def add_custom_pattern(self, name: str, pattern: str, severity: Severity,
                           description: str, false_positive_pattern: Optional[str] = None) -> None:
        """Add a custom pattern to the matcher."""
        fp_check = re.compile(false_positive_pattern) if false_positive_pattern else None
